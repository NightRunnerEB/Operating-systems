...
typedef struct {
  ...
  int read_index;         // текущая позиция чтения из буфера
  int write_index;        // текущая позиция записи в буфер
} shared_memory;

extern sem_t *writers_mutex; // Мьютекс для писателей
extern sem_t *readers_mutex; // Мьютекс для читателей
