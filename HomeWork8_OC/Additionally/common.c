...
sem_t *writers_mutex; // Определение мьютекса для писателей
sem_t *readers_mutex; // Определение мьютекса для читателей
...
void init(void) {
    ...
    writers_mutex = sem_open("/writers-mutex", O_CREAT, 0666, 1);
    if(writers_mutex == SEM_FAILED) {
        perror("Failed to open semaphor for writers_mutex");
        exit(EXIT_FAILURE);
    }
    
    readers_mutex = sem_open("/readers-mutex", O_CREAT, 0666, 1);
    if(readers_mutex == SEM_FAILED) {
        perror("Failed to open semaphor for readers_mutex");
        exit(EXIT_FAILURE);
    }
}
