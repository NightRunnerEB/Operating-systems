...
void sigfunc(int sig) {
    ...
    sem_close(writers_mutex);
    sem_unlink("/writers-mutex");
...
int main() {
    ...
    while (1) {
        ...
        if(sem_wait(writers_mutex) == -1) { // Блокировка мьютекса писателя
            perror("sem_wait: writers_mutex");
            exit(-1);
        }
        // Запись значения
        buffer->store[buffer->write_index] = rand() % 100;
        printf("Writer writes %d into buffer[%d]\n", buffer->store[buffer->write_index], buffer->write_index);
        buffer->write_index = (buffer->write_index + 1) % BUF_SIZE;
        
        if(sem_post(writers_mutex) == -1) { // Освобождение мьютекса писателя
            perror("sem_post: writers_mutex");
            exit(-1);
        }
        if(sem_post(full) == -1) {
            perror("sem_post");
